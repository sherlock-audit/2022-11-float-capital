  ...
  UUPSUpgradeable
{
  ...
  */
  function mint(address to, uint256 amount) external override(IAlphaFLT) onlyRole(MINTER_ROLE) {
    _mint(to, amount);
  }
  ...
  function totalSupply() public view virtual override(ERC20Upgradeable, IAlphaFLT) returns (uint256) {
    return ERC20Upgradeable.totalSupply();
  }

  function transfer(address recipient, uint256 amount) public virtual override(ERC20Upgradeable, IAlphaFLT) returns (bool) {
    return ERC20Upgradeable.transfer(recipient, amount);
  }

  function burnFrom(address account, uint256 amount) public virtual override(ERC20BurnableUpgradeable, IAlphaFLT) {
