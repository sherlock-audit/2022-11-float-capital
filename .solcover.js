...
let isUnitTests =
  !!process.env.DONT_RUN_INTEGRATION_TESTS &&
  process.env.DONT_RUN_INTEGRATION_TESTS.toUpperCase() == "TRUE";
let isIntegrationTests =
  !!process.env.DONT_RUN_UNIT_TESTS &&
  process.env.DONT_RUN_UNIT_TESTS.toUpperCase() == "TRUE";
if (isUnitTests) {
...
} else if (!isUnitTests && !isIntegrationTests) {
  // if it is neither then it is both (wierd logic but it works)
  // Don't do anything in this case
} else {
  throw Error(
    "Invalid config, don't set both 'DONT_RUN_INTEGRATION_TESTS' and 'DONT_RUN_UNIT_TESTS' to true"
  );
}
